/**
 * Amazing Slider - Responsive jQuery Slider and Image Scroller Copyright 2013
 * Magic Hills Pty Ltd All Rights Reserved Website: http://amazingslider.com
 * Version 2.3
 */
(function($) {
	$.fn.html5lightbox = function(options) {
		...
		if (inst.options.skinfolder.charAt(0) != "/"
			...
			inst.options.skinfolder = inst.options.jsfolder
		...
		inst.createMarkup = function() {
			...
			inst.$lightbox = jQuery("<div id='html5-lightbox' style='display:none;top:0px;left:0px;width:100%;height:100%;z-index:9999999;'>"
					...
					+ -/-/index.htm) no-repeat center center;'></div>"
					...
					+ "</div>" + "</div>");
			inst.$lightbox.css({
				position : inst.options.supportCSSPositionFixed ? "fixed"
						: "absolute"
			});
		...
		inst.showImage = function(elem, imgW, imgH) {
			...
			inst.resizeLightbox(sizeObj.w, sizeObj.h, true, function() {
				...
				inst.$image.html("<img src='" + elem[ELEM_HREF] + "' width='"
						+ sizeObj.w + "' height='" + sizeObj.h + "' />");
				inst.$elem.fadeIn(inst.options.fadespeed, function() {
					inst.showData()
				})
			})
	...
	$.fn.amazingslider = function(options) {
		...
		AmazingSlider.prototype = {
			...
			readTags : function() {
				...
				$(".amazingslider-thumbnails", this.container).find("li").each(
						function(index) {
							var img = $("img", $(this));
							if (img.length > 0
			...
			createWatermark : function() {
				...
				if (this.options.watermarklink)
					html += "</a>";
				$(".amazingslider-watermark-" + this.id, this.container).html(
						html)
			},
			...
			createMarkup : function() {
				this.$wrapper = jQuery(""
						...
						+ this.id
						+ "'></div>"
						+ "<div class='amazingslider-watermark-"
						+ this.id
						+ "'></div>"
						+ "</div>"
			...
			playYoutubeVideo : function(href, autoplay) {
				...
				if (ASYouTubeIframeAPIReady && !ASPlatforms.isIE6()
					...
					/*var src = href + (href.indexOf("?") < 0 ? "?" : "&")
							+ "autoplay=1&output=embed&wmode=transparent&rel=0&autohide=1";*/
					
					
					var id  = href.match(/(\?v=|\/\d\/|\/embed\/|\/v\/|\.be\/)([a-zA-Z0-9\-\_]+)/)[2];
					var src = "https://www.youtube.com/embed/"+id+"?wmode=transparent&rel=0&autohide=1";
			...
			createText : function() {
				if (this.options.textstyle == "none")
					return;
				var instance = this;
				var $textWrapper = $(".amazingslider-text-wrapper-" + this.id,
						this.container);
				if (this.options.textstyle == "static") {
					$textWrapper.html("<div class='amazingslider-text-"
							+ this.id + "'>"
							+ "<div class='amazingslider-text-bg-" + this.id
							+ "'></div>" + "<div class='amazingslider-title-"
							+ this.id + "'></div>"
					...
					case "top":
						$textWrapper
								.css({
									left : "0px",
									top : "0px",
									"margin-top" : this.options.textpositionmarginstatic
											+ "px"
								});
						break;
					case "bottom":
						$textWrapper
								.css({
									left : "0px",
									bottom : "0px",
									"margin-bottom" : this.options.textpositionmarginstatic
											+ "px"
								});
						break;
					case "topoutside":
				...
				$("head").append(
						"<style type='text/css'>" + ".amazingslider-text-"
								+ this.id + " {" + this.options.textcss + "} "
								+ ".amazingslider-text-bg-" + this.id + " {"
								+ this.options.textbgcss + "} "
								+ ".amazingslider-title-" + this.id + " {"
								+ this.options.titlecss + "} "
								+ ".amazingslider-description-" + this.id
								+ " {" + this.options.descriptioncss + "} "
								+ "</style>");
				this.container
						.bind(
											...
											$textWrapper
													.fadeOut(function() {
														...
														case "topleft":
															...
															$textWrapper
																	.css({
																		margin : instance.options.textpositionmargintop
																				+ "px "
																				+ instance.options.textpositionmarginleft
																				+ "px"
																	});
															break;
														case "topright":
															...
															$textWrapper
																	.css({
																		margin : instance.options.textpositionmargintop
																				+ "px "
																				+ instance.options.textpositionmarginright
																				+ "px"
																	});
															break;
														case "bottomleft":
															...
															$textWrapper
																	.css({
																		margin : instance.options.textpositionmarginbottom
																				+ "px "
																				+ instance.options.textpositionmarginleft
																				+ "px"
																	});
														...
														case "fade":
															...
															$textBox
																	.delay(500)
																	.fadeIn(
																			instance.options.texteffectduration);
															break;
														case "slide":
															$textBox
																	.css({
															...
															$textBox
																	...
																	.animate(
																			...
																			instance.options.texteffectduration,
																			instance.options.texteffecteasing);
															break;
														default:
															$textBox.delay(500)
																	.show()
			...
			createStyle : function() {
				...
				if (this.options.borderradius > 0)
					styleCss += "border-radius:" + this.options.borderradius
				...
				if (this.options.showwatermark)
					if (this.options.watermarkstyle == "text"
							...
							&& this.options.watermarkimage.length > 0) {
						styleCss += ".amazingslider-watermark-" + this.id
			...
			createNav : function() {
				...
				if(this.options.shownavcontrol || typeof this.options.shownavcontrol == "undefined"){
			...
			createNavBullet : function(index) {
				...
				var $bullet = $("<div class='amazingslider-bullet-" + this.id
						...
						+ ";margin-" + marginF + ":" + spacing + "px;width:"
						+ w + "px;height:" + h + "px;cursor:pointer;'></div>");
													...
													+ Math
															.floor(this.width
																	...
																	* instance.options.navimageheight
																	...
																	/ 2) + "px";
										$bullet
												.append("<div style='display:block;position:absolute;width:"
														+ instance.options.navimagewidth
														+ "px;height:"
														+ instance.options.navimageheight
					...
					$.fn["bulletSelected" + this.id] = function() {
						...
						if (instance.options.navshowfeaturedarrow) {
							...
							if (instance.options.navdirection == "horizontal") {
								...
								if ($navContainer.width() < $bulletWrapper
										.width()
										&& !instance.pauseCarousel) {
									var m = Math
											.abs(isNaN(parseInt($bulletWrapper
													.css("margin-left"))) ? 0
													: parseInt($bulletWrapper
															.css("margin-left")));
									if (pos.left < m
											|| pos.left
													+ instance.options.navwidth > m
													+ $navContainer.width()) {
										var pl = -pos.left;
										...
										instance
												.updateCarouselLeftRightArrow(pl)
									}
								}
							} else {
								var l, t = pos.top
			...
			slideRun : function(index, playVideo) {
					...
					$(".amazingslider-img-box-" + this.id, this.container)
							...
							.html(
									"<div class='amazingslider-img-"
											+ this.id
											+ " ' style='display:block;position:absolute;left:0px;top:0px;width:100%;height:auto;overflow:hidden;'><img style='position:absolute;max-width:100%;width:100%;height:100%;left:0%;top:0%;' src='"
											+ this.elemArray[savedCur][ELEM_SRC]
											+ "' /></div>");
				...
				if (!this.isPaused && !this.tempPaused
						&& this.elemArray.length > 1)
						...
						.load(
								function() {
									...
									var $imgPrev = $(".amazingslider-img-"
											+ instance.id, instance.container);
									var $imgCur = $("<div class='amazingslider-img-"
											+ instance.id
											+ " ' style='display:block;position:absolute;left:0px;top:0px;width:100%;height:100%;overflow:hidden;'><img style='position:absolute;"
											+ (ASPlatforms.isIE678() ? "opacity:inherit;filter:inherit;"
													: "")
											+ "max-width:"
											+ ratio
		...
		this
				.each(function() {
					...
					var defaultOptions = {
						...
						watermarktextdefault : "ama" + "zings" + "lide"
								+ "r.com",
						watermarkimagedefault : "",
						watermarklinkdefault : "htt" + "p://ama" + "zingslide"
								+ "r.co" + "m?source=wm",
						watermarktargetdefault : "_blank",
						watermarkpositioncssdefault : "display:none;",
					...
					if (this.options.versionmark != "AMC" + "om") {
						this.options.showwatermark = window.location.href
								.indexOf("://" + "amazin" + "gslide" + "r.com") >= 0 ? false
								: true;
						this.options.watermarkstyle = this.options.watermarkstyledefault;
						this.options.watermarktext = "am" + "azin" + "gsli"
								+ "der.c" + "om";
...
(function($) {
	$.fn.amazingsliderTransition = function(id, $prev, $next, transition,
		...
		if ((effect == "threed" || effect == "threedhorizontal")
		...
		if (effect == "fade") {
			...
			$parent.threedHorizontalTransition(id, $prev, $next, $.extend({
				...
				easing : easing,
				direction : direction
			}, transition["threedhorizontal"]), callback,
	...
	$.fn.sliceTransition = function(id, $prev, $next, options, callback,
		...
		for (i = 0; i < options.slicecount; i++) {
			...
			$img.attr("style", $img.attr("style") + "; " + "max-width:" + w
					+ "px !important;");
			$imgSlice.append($img);
	...
	$.fn.threedTransition = function(id, $prev, $next, options, callback,
		...
		for (i = 0; i < options.slicecount; i++) {
			...
			$nextImg.attr("style", $nextImg.attr("style") + "; " + "max-width:"
					+ w + "px !important;");
			...
			$curImg.attr("style", $curImg.attr("style") + "; " + "max-width:"
					+ w + "px !important;");
	...
	$.fn.threedHorizontalTransition = function(id, $prev, $next, options,
		...
		var sliceH = Math.ceil(h / options.slicecount);
		var $cubeWrapper = $("<div class='amazingslider-img-cube-wrapper-"
				+ id
				+ " ' style='display:block;position:absolute;left:0%;top:0%;width:100%;height:100%;'></div>");
		$parent.append($cubeWrapper);
	...
	$.fn.shuffleTransition = function(id, $prev, $next, options, callback,
		...
		for (i = 0; i < options.rowcount; i++)
			for (j = 0; j < options.columncount; j++) {
				...
				$imgNext.attr("style", $imgNext.attr("style") + "; "
				...
				$imgPrev.attr("style", $imgPrev.attr("style") + "; "
						+ "max-width:" + w + "px !important;");
